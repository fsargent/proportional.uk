...
export function calculateParliamentPoints(
  ...
  while (points.length < totalPoints) {
    ...
    const currRadStep = Math.atan(
      (spacingFactor * seatRadius) / currentRowRadius,
    );
    const rowSeats = Math.min(
      Math.floor((currentRowEndRad - currentRowStartRad) / currRadStep),
      totalPoints - points.length,
    );
    const roundedRadStep =
      rowSeats > 0 ? (currentRowEndRad - currentRowStartRad) / rowSeats : 0;
    ...
    for (let currSeat = 0; currSeat < rowSeats; currSeat++) {
      ...
      points.push({
        ...
        y:
          graphicHeight -
          Math.sin(currentAngle) * currentRowRadius -
          seatRadius,
      });
    }
    currentRow++;
...
export function drawParliamentChart(
  ...
  const containerWidth =
    canvas.parentElement?.offsetWidth || (isCompact ? 300 : 600);
  ...
  const seatPositions = calculateParliamentPoints(
    totalSeats,
    chartOptions,
    canvasWidth,
  );
