...
export function buildDistrictContours(
  seats: HexSeat[],
    ...
    for (const edge of edgeMap.values()) {
      ...
      while (true) {
        ...
        const candidate = options.find(
          (point) => !used.has(`${pointKey(next)}>${pointKey(point)}`),
        );
        if (!candidate) break;
        current = next;
        next = candidate;
        if (
          pointKey(current) === pointKey(ring[0]) &&
          pointKey(next) === pointKey(ring[1])
        ) {
          break;
        }
      ...
      const deduped = ring.filter(
        (point, index) =>
          index === 0 || pointKey(point) !== pointKey(ring[index - 1]),
      );
      if (deduped.length >= 3) {
        const open = deduped[deduped.length - 1];
        const closed =
          pointKey(open) === pointKey(deduped[0])
            ? deduped.slice(0, -1)
            : deduped;
