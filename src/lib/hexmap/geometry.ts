...
export function oddRNeighbours(q: number, r: number): Array<{ q: number; r: number }> {
	const parity = Math.abs(r % 2) === 0 ? 'even' : 'odd';
	return ODD_R_DIRECTIONS[parity].map(([dq, dr]) => ({ q: q + dq, r: r + dr }));
}

export function oddRToPixel(q: number, r: number, size = HEX_SIZE): { x: number; y: number } {
	const x = Math.sqrt(3) * size * (q + 0.5 * (r & 1));
	const y = size * 1.5 * -r;
	return { x, y };
}

export function normaliseSeatPixels(seats: HexSeat[], size = HEX_SIZE): SeatPixelPosition[] {
...
export function hexPolygonPoints(x: number, y: number, size = HEX_SIZE): string {
