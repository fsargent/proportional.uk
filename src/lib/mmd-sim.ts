...
export function simulateGroupedDistricts(
  ...
  const districtSummaries: DistrictSimulation[] = districts.map((district) => {
    ...
    const seatAllocation = allocateSainteLague(
      partyVotes,
    ...
    const winningParties = Object.entries(seatAllocation)
      ...
      .sort(
        (a, b) =>
          b[1] - a[1] || (partyVotes[b[0]] ?? 0) - (partyVotes[a[0]] ?? 0),
      )
      .map(([party]) => party);
  ...
  const allParties = [
    ...new Set([...Object.keys(nationalVotes), ...Object.keys(nationalSeats)]),
  ];
  ...
  const loosemoreHanby =
    nationalPartyTotals.reduce(
      (sum, party) =>
        sum + Math.abs(party.voteShare * 100 - party.seatShare * 100),
      0,
    ) / 2;
  ...
  return {
    ...
    estimatedCandidatesPerBallot:
      districtSummaries.reduce(
        (sum, district) => sum + district.candidateCount,
        0,
