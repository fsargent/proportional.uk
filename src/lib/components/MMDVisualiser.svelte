<script lang="ts">
	...
	let {
		title = 'How district size reshapes Westminster',
		intro = 'This tool groups current Westminster constituencies into larger multi-member districts, then shows how district magnitude changes local scale, party balance, ballot pressure, and practical design choices.',
		showHero = true,
	...
	function getInitialGroupingIndex() {
		return Math.max(0, groupingOptions.indexOf(initialMagnitude));
	}
...
<section class="mmd-page">
	...
	<div class="control-panel">
		...
		<p class="summary-note">
			Green band marks a common working range for local multi-member districts. The seat split shown here is illustrative rather than a final count for any specific voting method.
		</p>
	</div>
	...
	<div class="map-and-tooltip">
		...
		<div class="tooltip-card">
			{#if hoveredDistrict}
				...
				{/if}
				<p>
					<strong>Illustrative Sainte-Laguë seat split:</strong>
			...
			{:else}
				<h3>Hover or click a district</h3>
				<p>Move over a grouped district to preview it, or click to keep it selected while you read its illustrative seat balance.</p>
			{/if}
		</div>
	</div>
	...
	<section class="results-grid">
		<div class="result-card">
			<h3 class="section-header">Illustrative seat outcome</h3>
	...
	{#if showAssumptions}
		<section class="assumptions-row">
			<div class="result-card">
				<h3 class="section-header">What this demonstrator assumes</h3>
				<ul>
					...
					<li>The district lines shown here are generated by an algorithm. Their exact boundaries are illustrative.</li>
					<li>Current constituency vote totals are summed into each grouped district.</li>
					<li>Seats are shown using <strong>Sainte-Laguë</strong> as a proportional benchmark.</li>
					<li>Different multi-member district methods can share the same district map while using different ballots and counts.</li>
					<li>This tool isolates the district-magnitude question first.</li>
				</ul>
			</div>
		</section>
	{/if}
...
<style>
	.mmd-page { display: grid; gap: 1.75rem; padding-bottom: 2rem; }
	.hero-block { max-width: 54rem; }
	.hero-block h2 { margin: 0.25rem 0 0.75rem; }
	.intro { font-size: 1.05rem; max-width: 50rem; margin: 0; }
	.control-panel, .result-card, .tooltip-card {
		padding: 1.25rem; border-radius: var(--radius-lg); border: 1px solid var(--border-color);
		background: var(--surface-subtle-gradient); box-shadow: var(--shadow-soft);
	}
	.view-toggle { display: flex; gap: 0.65rem; flex-wrap: wrap; }
	.view-toggle button {
		border: 1px solid var(--border-color); background: var(--surface-color); border-radius: 999px;
		padding: 0.7rem 1rem; font: inherit; font-weight: 600; cursor: pointer;
	}
	.view-toggle button.selected { background: var(--header-bg); color: var(--text-inverse); border-color: var(--header-bg); }
	.summary-strip p, .result-card ul, .summary-note { margin: 0; }
	...
	.summary-strip { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 0.75rem; padding-top: 1rem; border-top: 1px solid var(--border-color); }
	.summary-note { color: var(--text-soft); font-size: 0.95rem; }
	.map-and-tooltip { display: grid; gap: 1rem; align-items: start; }
	.tooltip-card h3 { margin-top: 0; margin-bottom: 0.75rem; }
	.result-card .section-header { margin: 0 0 1rem 0; padding-bottom: 0.7rem; }
	.selection-banner {
		display: flex; align-items: center; justify-content: space-between; gap: 1rem; margin-bottom: 0.9rem;
		padding: 0.65rem 0.85rem; border-radius: var(--radius-md); background: color-mix(in srgb, var(--header-bg) 12%, transparent); border: 1px solid var(--accent-border);
	}
	.selection-banner p { margin: 0; color: var(--accent-text); }
	.selection-clear {
		border: 1px solid var(--border-color); background: var(--surface-color); border-radius: 999px;
	...
	.results-grid, .assumptions-row { display: grid; gap: 1rem; }
</style>
