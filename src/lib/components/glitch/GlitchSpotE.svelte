<script lang="ts">
	...
	function buildTimeline(opts: { repeat: number; repeatDelay: number; paused: boolean }) {
		...
		tl.set(q('.scene'), { autoAlpha: 0 })
			...
			.fromTo(q1('.scene-1 .bg-image'), { scale: 1.12, opacity: 0 }, { scale: 1, opacity: 0.38, duration: 0.65 })
			.fromTo(
			...
			.to(q1('.scene-1'), { x: -10, duration: 0.04 })
			.to(q1('.scene-1'), { x: 6, duration: 0.04 })
			...
			.fromTo(q1('.scene-2 .bg-image'), { scale: 1.08, opacity: 0 }, { scale: 1, opacity: 0.28, duration: 0.45 })
			.fromTo(q1('.scene-2 .panel'), { scale: 0.97, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.28 }, '-=0.2')
...
<div class="video-frame" class:export-mode={mode === 'export'} bind:this={root}>
	...
	<div class="scene scene-1">
		<img class="bg-image" src="/images/glitch/ERS_No_Voice_Asset_003.png" alt="" aria-hidden="true" />
		<div class="kicker">Politics Is A Closed Club</div>
		<div class="headline headline-1">
			<div class="clip"><span class="glitch-line">THE SYSTEM</span></div>
			<div class="clip"><span class="glitch-line acid flash-word">WORKS</span></div>
			<div class="clip"><span class="glitch-line">FOR THEM</span></div>
		</div>
		<div class="subhead">Let's make it work for us.</div>
	</div>

	<div class="scene scene-2">
		<img class="bg-image" src="/images/glitch/ERS_No_Voice_Asset_003.png" alt="" aria-hidden="true" />
		<div class="panel stat-panel">
			<div class="tiny-label">Name's Not Down</div>
			<div class="stat-line">YOU'RE NOT</div>
			<div class="stat-line acid">COMING IN</div>
			<div class="microcopy">People feel like politics is a closed club — and they're right. Let's change that for good.</div>
		</div>
	</div>

	<div class="scene scene-3">
		<div class="tiny-label">Rewrite The Rules</div>
		<div class="fix">THE SYSTEM'S<br />RIGGED</div>
		<div class="sub">Let's rewrite the rules.</div>
		<div class="patch-note">We're not just changing the players. We're changing the game.</div>
		<div class="grid">
	...
	<div class="scene scene-4">
		...
		<div class="cta-line">TAKE BACK</div>
		<div class="cta-line acid">CONTROL</div>
		<a class="url" href="https://www.open-britain.co.uk/ncer" target="_blank" rel="noreferrer"
			>open-britain.co.uk/ncer</a
		>
		<div class="variant-tag">Politics is broken. It's time we take back control.</div>
	</div>
</div>
