<script lang="ts">
	...
	function buildTimeline(opts: { repeat: number; repeatDelay: number; paused: boolean }) {
		...
		tl.set(q('.scene'), { autoAlpha: 0 })
			...
			.fromTo(q('.scene-1 .headline-line'), { yPercent: 110, opacity: 0 }, { yPercent: 0, opacity: 1, stagger: 0.08, duration: 0.46 })
			...
			.fromTo(q1('.stat-card'), { scale: 0.96, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.28 })
			...
			.fromTo(q('.scene-3 .fix-line'), { x: -30, opacity: 0 }, { x: 0, opacity: 1, stagger: 0.08, duration: 0.32 })
