<script lang="ts">
	...
	function buildTimeline(opts: { repeat: number; repeatDelay: number; paused: boolean }) {
		...
		tl.set(q('.scene'), { autoAlpha: 0 })
			...
			.fromTo(q1('.scene-1 .bg-image'), { scale: 1.15, opacity: 0 }, { scale: 1, opacity: 0.4, duration: 0.7 })
			.fromTo(
				q('.headline-1 .glitch-line'),
				{ yPercent: 120, opacity: 0 },
				{ yPercent: 0, opacity: 1, stagger: 0.1, duration: 0.5 },
				'-=0.35'
			)
			.fromTo(q1('.scene-1 .subhead'), { y: 12, opacity: 0 }, { y: 0, opacity: 0.92, duration: 0.24 }, '-=0.18')
			.to(q1('.scene-1'), { x: -8, duration: 0.04 })
			.to(q1('.scene-1'), { x: 5, duration: 0.04 })
			.to(q1('.scene-1'), { x: 0, duration: 0.06 })
			.to(q1('.scene-1 .flash-word'), {
				textShadow: '0 0 14px rgba(255,255,255,0.5), 5px 0 0 rgba(255,90,117,0.6), -5px 0 0 rgba(57,255,214,0.55)',
				duration: 0.06,
				repeat: 3,
				yoyo: true
			}, '-=0.1')
			.to(q1('.scene-1'), { autoAlpha: 0, duration: 0.18, delay: 1.4 })

			.set(q1('.scene-2'), { autoAlpha: 1 })
			.fromTo(q1('.scene-2 .bg-image'), { scale: 1.1, opacity: 0 }, { scale: 1, opacity: 0.3, duration: 0.5 })
			.fromTo(q1('.scene-2 .panel'), { scale: 0.97, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.28 }, '-=0.25')
			...
			.fromTo(q1('.scene-3 .bg-image'), { scale: 1.08, opacity: 0 }, { scale: 1, opacity: 0.25, duration: 0.45 })
			.fromTo(q1('.scene-3 .fix'), { clipPath: 'inset(0 100% 0 0)', opacity: 0.25 }, { clipPath: 'inset(0 0% 0 0)', opacity: 1, duration: 0.46 }, '-=0.2')
...
<div class="video-frame" class:export-mode={mode === 'export'} bind:this={root}>
	...
	<div class="scene scene-1">
		...
		<div class="headline headline-1">
			<div class="clip"><span class="glitch-line">THE OLD</span></div>
			<div class="clip"><span class="glitch-line acid flash-word">PLAY</span></div>
			<div class="clip"><span class="glitch-line acid flash-word">BOOK</span></div>
			<div class="clip"><span class="glitch-line">IS FAILING</span></div>
		</div>
		<div class="subhead">It's time for a rewrite.</div>
	</div>
	...
	<div class="scene scene-2">
		...
		<div class="panel stat-panel">
			...
			<div class="stat-line">THEY FOUGHT</div>
			<div class="stat-line dim">FOR QUEEN &amp; COUNTRY</div>
			<div class="stat-line acid">THEY DESERVE A VOICE</div>
			<div class="microcopy">Countries with different political systems have higher turnouts. People know their votes matter.</div>
		</div>
	</div>

	<div class="scene scene-3">
		<img class="bg-image" src="/images/glitch/ERS_Veteran_Asset_002.png" alt="" aria-hidden="true" />
		<div class="tiny-label">Transform Politics</div>
		<div class="fix">WE'VE DONE<br />IT BEFORE</div>
		<div class="sub">Let's transform politics again.</div>
		<div class="grid">
	...
	<div class="scene scene-4">
		...
		<div class="cta-line">TIME FOR</div>
		<div class="cta-line acid">A REWRITE</div>
		<a class="url" href="https://www.open-britain.co.uk/ncer" target="_blank" rel="noreferrer"
			>open-britain.co.uk/ncer</a
		>
		<div class="variant-tag">When people believe their vote matters, they show up.</div>
	</div>
</div>
